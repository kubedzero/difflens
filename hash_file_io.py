...
def read_hashes_from_file(input_path):
    ...
    input_path = common_utils.sanitize_and_validate_file_path(input_path)
...
def write_hashes_to_file(data_frame, output_path):
    # Pandas can handle relative paths, but handle relative->absolute conversion ourselves so we can print extra info
    output_path = common_utils.sanitize_and_validate_file_path(output_path)
